...
export function SummaryDetail() {
  ...
  if (!queueItem) return <main className="doctor-page"><p className="doctor-error" role="alert">This patient is not assigned to your clinical queue.</p><Link className="button button-outline" to="/doctor/queue">Return to queue</Link></main>;
  ...
  return <main className="doctor-page summary-page"><div className="doctor-page-heading"><div><p className="doctor-kicker">Patient summary</p><h1>{locked ? "Approved clinical record" : "Review intake summary"}</h1></div><button className="approve-button" disabled={locked} onClick={() => void approve()}>{locked ? "Approved" : "Approve record"}</button></div>
    ...
    <section className="final-diagnosis"><h2>Final Diagnosis &amp; Conclusion</h2><textarea value={diagnosis} disabled={locked || diagnosisSubmitted} onChange={(event) => setDiagnosis(event.target.value)} placeholder="Write the clinical conclusion" /><label>Diagnosis attachment (optional)<input type="file" disabled={locked || diagnosisSubmitted} onChange={(event) => setAttachmentName(event.target.files?.[0]?.name ?? "")} /></label>{attachmentName && <p>{attachmentName}</p>}<button disabled={locked || !diagnosis.trim() || diagnosisSubmitted} onClick={submitDiagnosis}>{diagnosisSubmitted ? "Diagnosis submitted" : "Submit Diagnosis"}</button>{diagnosisSubmitted && <p className="assignment-confirmation" role="status">Diagnosis submitted to the patient record.</p>}</section>
  </main>;
}
